...
def IDFT(X):
    ...
    kValues = np.arange(N) #values that k assumes in the sum
    for n in range(N): #an iteration for every n of the signal x(n)
        cSin = np.exp(1j * 2 * np.pi * kValues * n / N) #complexSine for the nTh value of x(n), for all k's
        x = np.append(x, sum(X * cSin)/N) #sum over k
    return x