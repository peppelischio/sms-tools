...
def optimalZeropad(x, fs, f):
    ...
    ## Your code here
    M = len(x)
    periodSamples = ceil(fs/f) #n of samples taken at fs for analyzing a period of x at frequency f
    periodsInX = float(M)/float(periodSamples)
    periodsToConsider = ceil(periodsInX) #next int of the number of periods of the signal x 
    N = periodSamples * periodsToConsider
    zeroesToAdd = N - M
    x = np.pad(x, (0, zeroesToAdd), 'constant', constant_values=(0))
    X = fft(x)
    mX = 20*np.log10(abs(X[:floor(N/2)+1]))
    print('len mX --> ', len(mX))
    return mX
