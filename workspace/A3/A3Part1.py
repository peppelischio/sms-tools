...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    M is to be calculated as the smallest positive integer for which the positive half of the DFT magnitude 
    spectrum has only two non-zero values. --> analyze positive part of spectrum, 
    take the first two non zeroes and take the array values in between
    """
    ## Your code here

    period1Samples = ceil(fs/f1) #n of samples taken at fs for analyzing a period of a signal at frequency f1
    period2Samples = ceil(fs/f2) #n of samples taken at fs for analyzing a period of a signal at frequency f2

    # M corresponds to LCM of the samples needed to take a full period of the two different signals
    LCM = (period1Samples * period2Samples) / gcd(period1Samples, period2Samples) 
    M = ceil(LCM)

    X = fft(x[:M])
    mX = 20*np.log10(abs(X[:floor(M/2)+1]))
    return mX